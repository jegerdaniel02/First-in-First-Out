...
time=1
maxtmp=0
for nums in range(0,highestID+1):
    
    for eachrow in allrows:
        if eachrow[0]==nums:
            rowssorted.append(eachrow)
            #satte rowsa i rowssorted i stigende rekkefølge etter kolonneA

def findFirstID(time):
    global maxtmp
    global gtime
    ...
    for i in range(0,maxtmp+1):
        for row in temp:
            if row[1]==i:
                ...
                #sortert tempsorted etter kolonneA av de som har arrived.
...
#    print("len of tmpsortd",len(tempsorted))   
    if len(tempsorted) >0:
        ...
        #currentPID er nå av de som arriva, den med lavest ID.
        #print(tempsorted)
        #temp=[]
